...
async function createPet(userId, name, img, age, weight, breed, sex, spieci, sterelized ){
    const sterelizedValue = sterelized === true || sterelized === "true" ? 1 : 0;
    const newPet = { id: uuidv4(), userId, name, img, age, weight, breed, sex, spieci, sterelize:sterelizedValue };

    await pool.query('INSERT INTO pets (id, user_id, name,img,age, weight, breed, sex, spieci, sterelized) VALUES (?,?,?,?,?,?,?,?,?,?)',
    [newPet.id, newPet.userId, newPet.name,newPet.img, newPet.age, newPet.weight, newPet.breed, newPet.sex, newPet.spieci, newPet.sterelize]);
    return newPet;
}
async function updatePet( petId, userId,  age, weight, breed, sex, spieci, sterelized) {
    ...
    if (verifyPet.length === 0) {
     ...
     await pool.query(
        'UPDATE pets SET   age = ?, weight = ?, breed = ?, sex = ?, spieci = ?, sterelized= ? WHERE user_id = ? AND id = ?',
        [ age, weight, breed, sex, spieci, sterelized, userId, petId]
    );
    const upPet = { petId, userId,  age, weight, breed, sex, spieci, sterelized};
    return upPet;
}
